...
module.exports = function (couchServer, dbName) {
    ...
    function ensureDesignDocsRegistered (callback) {
        async.parallel(
            [
                function (next) {
    ...
    database.open = function (callback) {
        async.waterfall([
            function (next) {
                couch.db.get(dbName, function (error) {
                    if (error) {
                        couch.db.create(dbName, next);
                    }
                    else {
                        //hack for db.create above
                        next(null, null, null);
                    }
                });
            },
            function (head, body, next) {
                couch = couch.use(dbName);
                ensureDesignDocsRegistered(next);
            }
        ], function (error) {
            if (error) {
                callback(error);
